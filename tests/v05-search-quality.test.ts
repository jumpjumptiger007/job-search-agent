...
describe("V0.5 Personio discovery",()=>{
  ...
  it("requires an explicit Personio career-site URL while accepting supported domains",async()=>{
    ...
    try {const [found]=await new PersonioAdapter("https://ACME.jobs.personio.de/careers",undefined,"de").discover();expect(requested).toBe("https://acme.jobs.personio.de/xml?language=de");expect(found.externalId).toBe("personio:acme:de-1");expect(()=>new PersonioAdapter("acme")).toThrow("explicit public");expect(()=>new PersonioAdapter("http://acme.jobs.personio.de")).toThrow("explicit public");expect(()=>new PersonioAdapter("https://acme.jobs.personio.com.example")).toThrow("Personio source");} finally {globalThis.fetch=original;}
  });
  ...
  it("skips malformed records and keeps Personio job URLs on the configured source",async()=>{
    ...
    try {const jobs=await new PersonioAdapter("https://acme.jobs.personio.com").discover();expect(jobs).toHaveLength(2);expect(jobs.map(found=>found.url)).toEqual(["https://acme.jobs.personio.com/job/external","https://acme.jobs.personio.com/job/broken"]);} finally {globalThis.fetch=original;}
  });
  ...
  it("treats empty or malformed position collections as no results",async()=>{
    ...
    try {await expect(new PersonioAdapter("https://acme.jobs.personio.com").discover()).resolves.toEqual([]);} finally {globalThis.fetch=original;}
  });
...
describe("V0.5 Germany eligibility",()=>{
  ...
  it.each([
    ...
    ["Switzerland-only","Switzerland-only",""],
    ["UK-only","UK-only",""],
    ...
    ["current foreign authorization","Remote","Applicants must be currently authorized to work in the United States."],
    ...
    ["US structured location","Anywhere in the United States",""],
