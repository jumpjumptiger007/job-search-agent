...
export function ingest(job:DiscoveredJob) {
 ...
 if(existing) { const canonical=selectCanonical({url:existing.canonical_url,sourceName:existing.canonical_source},{url:job.url,sourceName:job.sourceName}),upgrade=job.ats==="BA"&&existing.content_status==="INSUFFICIENT"&&job.contentStatus==="SUBSTANTIVE"; d.prepare("INSERT OR IGNORE INTO job_sources(job_id,url,source_name,is_canonical) VALUES(?,?,?,0)").run(existing.id,job.url,job.sourceName); if(canonical.url===job.url) d.prepare("UPDATE jobs SET canonical_url=?, canonical_source=?, ats=coalesce(?,ats), updated_at=CURRENT_TIMESTAMP WHERE id=?").run(job.url,job.sourceName,job.ats,existing.id); if(upgrade)d.prepare("UPDATE jobs SET jd_original=?, jd_hash=?, content_status='SUBSTANTIVE', score=NULL, score_explanation=NULL, status='DISCOVERED', updated_at=CURRENT_TIMESTAMP WHERE id=?").run(job.description,jdHash,existing.id); d.prepare("UPDATE job_sources SET is_canonical=CASE WHEN url=? THEN 1 ELSE 0 END WHERE job_id=?").run(canonical.url,existing.id); d.prepare("INSERT INTO audit_events(job_id,action,detail) VALUES(?,?,?)").run(existing.id,"SOURCE_DEDUPLICATED",JSON.stringify({url:job.url,key})); if(upgrade)d.prepare("INSERT INTO audit_events(job_id,action,detail) VALUES(?,?,?)").run(existing.id,"SOURCE_CONTENT_UPGRADED",JSON.stringify({source:job.sourceName})); const saved=d.prepare("SELECT * FROM jobs WHERE id=?").get(existing.id) as any;if(upgrade)writeJobFiles(saved); return {job:saved,created:false,upgraded:upgrade}; }
