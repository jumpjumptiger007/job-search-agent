...
export function ingest(job:DiscoveredJob) {
 ...
 let existing=d.prepare("SELECT jobs.* FROM job_sources JOIN jobs ON jobs.id=job_sources.job_id WHERE job_sources.url=?").get(job.url) as any;
