...
export class BundesagenturAdapter implements DiscoveryAdapter {
  ...
  private headers():Record<string,string>{return process.env.BA_API_KEY?{"X-API-Key":process.env.BA_API_KEY}:{};}
  private async detail(ref:string,title:string){try{const code=Buffer.from(ref).toString("base64"),res=await fetch(`https://rest.arbeitsagentur.de/jobboerse/jobsuche-service/pc/v4/jobdetails/${encodeURIComponent(code)}`,{headers:this.headers()});if(!res.ok)return{description:"",contentStatus:"INSUFFICIENT" as const};const d=await res.json() as any,description=jobText([d.stellenangebotsBeschreibung,d.stellenbeschreibung,d.beschreibung,d.aufgaben,d.anforderungen,d.qualifikationen].filter((v):v is string=>typeof v==="string").join("\n"));return{description,contentStatus:substantiveDescription(title,description)?"SUBSTANTIVE" as const:"INSUFFICIENT" as const};}catch{return{description:"",contentStatus:"INSUFFICIENT" as const};}}
  async discover(){const roles=[...new Set((this.p.roleFamilies||[]).map(role=>role.trim()).filter(Boolean))];if(!roles.length)return[];const location=this.p.location?.toLowerCase()==="germany"?"Deutschland":this.p.location||"Deutschland",found=new Map<string,any>();for(const role of roles){const u=new URL("https://rest.arbeitsagentur.de/jobboerse/jobsuche-service/pc/v6/jobs");u.searchParams.set("was",role);u.searchParams.set("wo",location);u.searchParams.set("umkreis",String(this.p.radiusKm||0));u.searchParams.set("size",String(this.limit));const res=await fetch(u,{headers:this.headers()});if(!res.ok)throw new Error(`BA returned ${res.status}; set BA_API_KEY if required`);for(const j of ((await res.json() as any).ergebnisliste||[])){const key=j.referenznummer?`ref:${j.referenznummer}`:`url:${j.externeURL||JSON.stringify(j)}`;if(!found.has(key))found.set(key,j);}}return Promise.all([...found.values()].slice(0,this.limit).map(async(j:any)=>{const title=j.stellenangebotsTitel||"Unknown role",detail=j.referenznummer?await this.detail(j.referenznummer,title):{description:"",contentStatus:"INSUFFICIENT" as const};return{company:j.firma||"Unknown employer",title,location:j.stellenlokationen?.[0]?.adresse?.ort,url:j.referenznummer?`https://www.arbeitsagentur.de/jobsuche/suche?angebotsart=1&was=${encodeURIComponent(j.referenznummer)}`:j.externeURL,sourceName:"Bundesagentur für Arbeit",ats:"BA",externalId:j.referenznummer?`ba:${j.referenznummer}`:undefined,...detail,postedAt:j.datumErsteVeroeffentlichung,discoveredVia:"Bundesagentur für Arbeit"};}));}
}
...
export async function scoreForReview(job:any,profile:CandidateProfile,config:ScoringConfig){if(job.content_status==="INSUFFICIENT"){const explanation="Source content is insufficient for deterministic scoring or tailoring. The job remains available for human Review.";db().prepare("UPDATE jobs SET score=NULL,score_explanation=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(explanation,job.id);db().prepare("INSERT INTO audit_events(job_id,action,detail) VALUES(?,?,?)").run(job.id,"SOURCE_CONTENT_INSUFFICIENT",JSON.stringify({source:job.canonical_source}));return{score:null,generated:false};}const r=scoreJob(job.jd_original,profile,config.hardFilters);db().prepare("UPDATE jobs SET score=?,score_explanation=?,status='ANALYZED',updated_at=CURRENT_TIMESTAMP WHERE id=?").run(r.score,JSON.stringify(r),job.id);db().prepare("INSERT INTO audit_events(job_id,action,detail) VALUES(?,?,?)").run(job.id,"SCORED",JSON.stringify({score:r.score,autoGenerateDisabled:true}));return r;}
