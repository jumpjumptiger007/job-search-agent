...
export class PersonioAdapter implements DiscoveryAdapter {
  ...
  async discover(){const endpoint=new URL("/api/v1/recruiting/positions",this.base);endpoint.searchParams.set("language",this.language);const res=await fetch(endpoint);if(!res.ok)throw new Error(`Personio returned ${res.status}`);const payload=await res.json() as {data?:PersonioPosition[];positions?:PersonioPosition[]}|PersonioPosition[]|null,rows=Array.isArray(payload)?payload:Array.isArray(payload?.data)?payload.data:Array.isArray(payload?.positions)?payload.positions:[];return rows.slice(0,this.limit).flatMap((position:unknown)=>{if(!position||typeof position!=="object")return[];const item=position as PersonioPosition,id=item.id===undefined?undefined:String(item.id);if(!id)return[];let candidate:URL;try{candidate=new URL(item.jobUrl||item.url||`/job/${encodeURIComponent(id)}`,this.base);}catch{candidate=new URL(`/job/${encodeURIComponent(id)}`,this.base);}const url=candidate.protocol==="https:"&&candidate.hostname===this.base.hostname?candidate.href:new URL(`/job/${encodeURIComponent(id)}`,this.base).href;const office=typeof item.office==="string"?item.office:item.office?.name;const location=office||(typeof item.location==="string"?item.location:item.location?.name);return[{company:this.company||this.tenant,title:item.name||item.title||"Unknown role",location,url,sourceName:"Personio",ats:"Personio",externalId:`personio:${this.tenant}:${id}`,description:jobText(item.description||""),postedAt:item.createdAt||item.created_at,workModel:item.workplaceType||item.employmentType,discoveredVia:"configured ATS"}];});}
}
