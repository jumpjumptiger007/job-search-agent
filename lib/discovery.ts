...
type PersonioPosition={id?:string|number;name?:string;title?:string;description?:string;office?:{name?:string}|string;location?:{name?:string}|string;url?:string;jobUrl?:string;createdAt?:string;created_at?:string;workplaceType?:string;employmentType?:string};
...
export function isGermanyEligible(job:DiscoveredJob){
  ...
  if(clearlyForeignRequirement(all,foreignResidency)||clearlyForeignRequirement(all,requiredForeignResidency)||clearlyForeignRequirement(all,foreignAuthorization)||clearlyForeignRequirement(all,existingForeignAuthorization)||clearlyForeignRequirement(all,requiredForeignAuthorization))return false;
  const normalizedLocation=location.trim();
  if(normalizedLocation&&incompatibleCountry.test(normalizedLocation)&&!compatibleRegion(normalizedLocation)&&!/remote|hybrid|multiple|global|world/i.test(normalizedLocation)&&explicitCountry.test(normalizedLocation))return false;
  return true;
}
...
export class PersonioAdapter implements DiscoveryAdapter {
  ...
  constructor(source:unknown,private limit?:number,private language="en",private company?:string){const raw=typeof source==="string"?source.trim():"";if(!raw)throw new Error("Personio source is required");const candidate=/^https?:\/\//i.test(raw)?raw:/\.jobs\.personio\./i.test(raw)?`https://${raw}`:`https://${raw}.jobs.personio.com`;const url=new URL(candidate);if(url.protocol!=="https:"||url.username||url.password||!personioHost.test(url.hostname))throw new Error("Personio source must be a public <tenant>.jobs.personio.<tld> URL or tenant");this.base=new URL(`${url.protocol}//${url.host}`);this.tenant=tenantFor(this.base.href,"Personio")!;}
  async discover(){const endpoint=new URL("/api/v1/recruiting/positions",this.base);endpoint.searchParams.set("language",this.language);const res=await fetch(endpoint);if(!res.ok)throw new Error(`Personio returned ${res.status}`);const payload=await res.json() as {data?:PersonioPosition[];positions?:PersonioPosition[]}|PersonioPosition[],rows=Array.isArray(payload)?payload:payload.data||payload.positions||[];return rows.slice(0,this.limit).flatMap(position=>{const id=position.id===undefined?undefined:String(position.id);if(!id)return[];const url=new URL(position.jobUrl||position.url||`/job/${id}`,this.base).href;const office=typeof position.office==="string"?position.office:position.office?.name;const location=office||(typeof position.location==="string"?position.location:position.location?.name);return[{company:this.company||this.tenant,title:position.name||position.title||"Unknown role",location,url,sourceName:"Personio",ats:"Personio",externalId:`personio:${this.tenant}:${id}`,description:jobText(position.description||""),postedAt:position.createdAt||position.created_at,workModel:position.workplaceType||position.employmentType,discoveredVia:"configured ATS"}];});}
}
...
export async function runDiscovery(configPath="config/search.yaml"){
  ...
  const stat={seen:0,newJobs:0,duplicates:0,failures:configurationErrors.length,blocked:0,scored:0,generated:0,errors:configurationErrors},profile=loadProfile(),score=profile?loadScoringConfig():undefined;
  let remaining=max;
  ...
  d.prepare("UPDATE discovery_runs SET ended_at=CURRENT_TIMESTAMP,sources_attempted=?,jobs_seen=?,new_jobs=?,duplicates=?,failures=?,blocked_sources=?,errors=? WHERE id=?").run(adapters.map(a=>a.name).join(", "),stat.seen,stat.newJobs,stat.duplicates,stat.failures,stat.blocked,stat.errors.join("\n"),run.lastInsertRowid);
